...
int test(void) {
    ...
    string txt = config.dataset + "\\test.txt";
...
int run(void) {
    Config &config = Config::GetInstance();
    FILE *fd = fopen((config.dataset + "\\test.txt").c_str(), "r");
    assert(fd);
    int N;
    ...
    for (int i = 0; i < N; i++) {
		...
		fscanf(fd, "%s", img_path_1);
		fscanf(fd, "%s", img_path_2);
		fscanf(fd, "%s", img_path_3);

		strcat(img_path, img_path_1);
		strcat(img_path, " ");
		strcat(img_path, img_path_2);
		strcat(img_path, " ");
		strcat(img_path, img_path_3);
